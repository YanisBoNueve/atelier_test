...
FROM `projet.atelier.analytics.training_sales`
UNION ALL
SELECT 'training_user_progress_snapshot', COUNT(*)
FROM `projet.atelier.analytics.training_user_progress_snapshot`
UNION ALL
SELECT 'user_homework_submission', COUNT(*)
FROM `projet.atelier.analytics.user_homework_submission`
;
...
LEFT JOIN `projet.atelier.analytics.training_user_progress_snapshot` AS ups
...
LEFT JOIN `projet.atelier.analytics.training_sales` AS ts
  ON ts.user_id = ups.user_id
